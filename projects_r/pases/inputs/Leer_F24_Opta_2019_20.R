...
##########################################
######################################################################################
# PLANTILLAS 2019-2020
######################################################################################

x <- read_xml("srml-23-2019-squads.xml")
...
##########################################
##########################################
# ATLETICO MADRID GETAFE 2019-20
##########################################
##########################################

## CAMBIAR AQUI POR EL NOMBRE DE FICHERO

### partido 2019-20 Atletico Madrid  - Getafe

eventos <- leer_f24("f24-23-2019-1074817-eventdetails.xml")
...
regates_vitolo<- eventos %>%
  filter(type_id==3 & Name == 'Vitolo')
...
# regates Vitolo del Atlético Madrid
crear_Campo()+
  geom_point(data=regates_vitolo,aes(x=x,y=y, color=resultado), size=3)+
  geom_text_repel(data=regates_vitolo,aes(x=x,y=y, label=Name))+
