...
regates_atletico <- eventos %>%
  filter(type_id==3 & team_id==175)


regates_carrasco<- eventos %>%
  filter(type_id==3 & Name == 'Yannick Carrasco')
...
# regates Carrasco Madrid
crear_Campo()+
  geom_point(data=regates_carrasco,aes(x=x,y=y, color=resultado), size=3)+
  geom_text_repel(data=regates_carrasco,aes(x=x,y=y, label=Name))+
...
crear_Campo()+
  geom_point(data=regates_atletico,aes(x=x,y=y, color=resultado), size=3)+
  geom_text_repel(data=regates_atletico,aes(x=x,y=y, label=Name))+
  scale_colour_manual(values = c("No Completado" = "red", "Completado" = "green"))
...
pases_atletico <- eventos %>%
  filter(type_id==1 & team_id==175)


pases_carrasco<- eventos %>%
...
####################
# GRAFICA PASES
# Se dibuja un punto con el inicio del pase columnas x,y
# Se dibuja un segmento con el inicio del pase x,y  junto con el destino x,y columnas `140`y`141`
####################
...
crear_Campo()+
  ...
  geom_segment(data=pases_carrasco,aes(x=x,y=y,xend = `140`, yend = `141`, colour=resultado), size=1)+
  scale_colour_manual(values = c("No Completado" = "red", "Completado" = "green"))



## Opcion 2 segmento con flechas

crear_Campo()+
  geom_segment(data=pases_carrasco, 
               aes(x = x, y = y, xend =`140`, yend = `141`,color=resultado),
               position="nudge", arrow = arrow(length = unit(0.1, "inches") ) , size=1 )+
  scale_colour_manual(values = c("No Completado" = "red", "Completado" = "green"))
